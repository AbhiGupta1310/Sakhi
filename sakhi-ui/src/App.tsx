...
const SakhiApp: React.FC = () => {
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'greeting',
      role: 'sakhi',
      content: "Namaste 🙏 I am Sakhi, your AI legal companion. I'm here to help you understand your rights and the law in simple, clear language. How can I assist you today?"
    }
  ]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError("I'm sorry, I'm having trouble connecting right now. Please try again in a moment.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 text-slate-800 font-sans">
      ...
      <header className="bg-white px-6 py-4 shadow-sm border-b border-slate-200 flex items-center gap-3 z-10 sticky top-0">
        ...
        <div>
          <h1 className="text-xl font-bold bg-gradient-to-r from-orange-600 to-amber-500 bg-clip-text text-transparent">Sakhi</h1>
          <p className="text-xs text-slate-500 font-medium">Your trusted AI legal companion</p>
        </div>
      </header>
              ...
              >
                <div className="flex gap-3 flex-row max-w-[75%]">
                  ...
                  <div className="px-5 py-4 rounded-2xl shadow-sm bg-white border border-slate-100 rounded-tl-sm flex items-center gap-1.5 h-[56px]">
                    <motion.div className="w-2 h-2 rounded-full bg-slate-300" animate={{ y: [0, -5, 0] }} transition={{ repeat: Infinity, duration: 1, delay: 0 }} />
                    <motion.div className="w-2 h-2 rounded-full bg-slate-300" animate={{ y: [0, -5, 0] }} transition={{ repeat: Infinity, duration: 1, delay: 0.2 }} />
                    <motion.div className="w-2 h-2 rounded-full bg-slate-300" animate={{ y: [0, -5, 0] }} transition={{ repeat: Infinity, duration: 1, delay: 0.4 }} />
                  </div>
                </div>
              </motion.div>
            )}
      ...
      <footer className="bg-white border-t border-slate-200 p-4 sticky bottom-0 z-10">
        <div className="max-w-3xl mx-auto w-full">
          <form onSubmit={handleSendMessage} className="relative flex items-end shadow-sm border border-slate-300 rounded-2xl bg-slate-50 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500 transition-shadow">
            <textarea
              value={inputValue}
          ...
          <p className="text-center text-xs text-slate-400 mt-3 font-medium">
            Sakhi is an AI companion to help you understand Indian law. It does not replace professional legal advice.
          </p>
        </div>
      </footer>
    </div>
